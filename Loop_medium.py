...
def boucle_moyen(mute, language):
    ## ----- Son ----- ##
    mixer.init()
...
## ----- Classe pour creer les cases ----- ##
    ...
    def random_bombes():
        bombes_choisi = random.sample(Case_m.total, 14)
...
## ----- Création des canevas -----##
    # Fond
    Main = Canvas(fen, width=850, height=590, bg='#a39193')
    Main.grid(row=0, column=0, columnspan=1, padx=3, pady=3)

    bordure = Frame(fen, bg='#f6e0b5', width=617, height=475)
    bordure.place(x=120, y=100)

    grille = Frame(fen)
    grille.place(x=137, y=113)

## ---- Création des informations -----##
    # Titre
    titre = Label(fen, text=" - MOYEN - ")
    ...
    nb_bombe = Label(fen, text=" Bombes : 14 ")
    ...
    nb_case_restantes = Label(fen, text=" Cases restantes : 50 ")
