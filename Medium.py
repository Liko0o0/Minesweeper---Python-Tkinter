...
m_path = appdata_path / "time_medium.txt"
...
## ----- Creation of the window ----- ##
    window = Tk()
    if language == 'french':
        window.title('Démineur (Moyen)')
    else:
        window.title('Minesweeper (Medium)')
    window_center(860, 600)
...
## ----- Class to create the cell ----- ##
    class Cell_m:
        ...
        def create_cell_btn(self):
            btn = Button(grid, width=9, height=3, bg="#aa6f73")
        ...
        def cell_around(self):
            ...
            cell_around = [
                Case for Case in cell_around if Case is not None]
            return cell_around
        ...
        def show_cell(self):
            if not self.is_discovered:
                ...
                if nb == 0:
                    ...
                    self.btn_cell.configure(fg="#0037ff", bg='#d3927b')
                elif nb == 2:
                    self.btn_cell.configure(fg="#119b00", bg='#d3927b')
                elif nb == 3:
                    self.btn_cell.configure(fg="#ac5a00", bg='#d3927b')
                elif nb == 4:
                    self.btn_cell.configure(fg="#d41a00", bg='#d3927b')
                elif nb == 5:
                    self.btn_cell.configure(fg="#bb00bb", bg='#d3927b')
                elif nb >= 6:
                ...
                if language == 'french':
                    nb_remaining_cell.configure(
                        text=f" Cases restantes : {Cell_m.cell_nb_m} ")
                else:
                    nb_remaining_cell.configure(
                        text=f" Cell left : {Cell_m.cell_nb_m} ")
...
## ----- Canvas creation -----##
    ...
    border = Frame(window, bg='#f6e0b5', width=617, height=475)
    border.place(x=120, y=100)

    grid = Frame(window)
    grid.place(x=137, y=113)
