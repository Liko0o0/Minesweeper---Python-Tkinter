...
chemin_musique = Path(__file__).parent / "Sound/music_game.mp3"
son_case = Path(__file__).parent / "Sound/cell_click.mp3"
son_marque = Path(__file__).parent / "Sound/cell_flag.mp3"
son_explosion = Path(__file__).parent / "Sound/explosion.mp3"
son_win = Path(__file__).parent / "Sound/win.mp3"
...
def boucle_facile(mute, language):
...
## ----- Creation de la fenetre ----- ##
    fen = Tk()
    if language == 'french':
        fen.title('Démineur (Facile)')
    else:
        fen.title('Minesweeper (Easy)')
    centrer_fen(710, 480)
...
## ----- Classe pour creer les cases ----- ##
    class Case:
        ...
        def entree(self, event):  # Event provient de la methode bind
            global game_over
            if not self.est_decouvert and not self.marquer and not game_over:
                self.case_btn.configure(bg="#996367")

        def sortie(self, event):  # Event provient de la methode bind
            global game_over
            if not self.est_decouvert and not self.marquer and not game_over:
            ...
            if not self.marquer:
                ...
                if self.est_bombe:
                    ...
                    if Case.nb_case == 0:
                        ...
                        if rejouer == True:
                            fen.destroy()
                            boucle_facile(mute, language)
                        elif rejouer == False:
                            import Main
                            Main.musique_en_cour = 0
                            fen.destroy()
                            Main.menu(mute, language)

        def recuperer_coord_case(self, x, y):
            for case in Case.total:
            ...
            if not self.est_decouvert:
                ...
                if language=='french':
                    nb_case_restantes.configure(
                        text=f" Cases restantes : {Case.nb_case} ")
                else:
                    nb_case_restantes.configure(
                        text=f" Cell left : {Case.nb_case} ")
    ...
    def random_bombes():
        bombes_choisi = random.sample(Case.total, 7)
...
## ----- Création des canevas -----##
    # Fond
    Main = Canvas(fen, width=700, height=470, bg='#a39193')
    Main.grid(row=0, column=0, columnspan=1, padx=3, pady=3)

    bordure = Frame(fen, bg='#f6e0b5', width=472, height=362)
    bordure.place(x=120, y=95)

    grille = Frame(fen)
    grille.place(x=137, y=108)

## ---- Création des informations -----##
    # Titre
    titre = Label(fen, text=" - FACILE - ")
    ...
    temps.grid(row=0, column=0, columnspan=2, padx=40, pady=60, sticky=NE)
    temps.config(font=("Small fonts", 18), bg='#E1CCCE', relief=RIDGE)
    ...
    nb_bombe = Label(fen, text=" Bombes : 7 ")
    nb_bombe.grid(row=0, column=0, columnspan=2,
                  padx=15, pady=15, sticky=NW)
    nb_bombe.config(font=("Small fonts", 14), bg='#E1CCCE', relief=RIDGE)

    nb_case_restantes = Label(fen, text=" Cases restantes : 29 ")
    ...
    if language == 'english':
        titre.config(text=' - EASY - ')
        nb_bombe.config(text=' Mines : 7 ')
        nb_case_restantes.config(text=' Cell left : 29 ')
